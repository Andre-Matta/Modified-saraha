...
string ContactsPage::getUsername(vector<User>users, double id) {
...
double ContactsPage::getContactId(vector<User>users, string username) {
    for (int i = 0; i < users.size(); i++) {
...
bool ContactsPage::findUsername(vector<User>users, string username
    , string currentUsername)
{
    if (username == currentUsername) {
        cout << "you can't add your contact";
    }
    else {
        for (int i = 0; i < users.size(); i++) {
            if (users[i].get_username() == username) {
...
vector<string> ContactsPage::search(string username, int currentUser_id) {
...
void ContactsPage::viewUserContacts(Text contacts[], RenderWindow& Window, vector<string> v, bool myContacts) {
    Font font;
    font.loadFromFile("Fonts\\Basic-Regular.ttf");
...
void ContactsPage::viewMyContacts(Text contacts[], RenderWindow& Window, vector<string> v, bool myContacts, int p) {
    Font font;
    font.loadFromFile("Fonts\\Basic-Regular.ttf");
    if (p >= 6) {
        ...
        for (int i = p - 6; i < p; i++) {
            ...
            Window.draw(contacts[i]);
            contacts[p - 1].setFillColor(Color(61, 144, 227));
        }
    }
    else {
        int size;
...
void ContactsPage::sendmsg(double sender, string reciever, string msg, string senderUsername) {
    long long sendId = sender;
    bool favorite = false;
    string senderID = to_string(sendId);
...
int ContactsPage::viewSend(User curruser)
{
    ...
    Sprite back_button_sprite, messages, contacts, text_bar_sprite, backgroundSprite, textBar, searchtxt, undoButton,
        sendButton, addButton, chat, contact_button;
    ...
    contact_button.setTexture(buttons);
    ...
    contact_button.setTextureRect(IntRect(252, 0, 250, 73));
    ...
    contact_button.setPosition(30, 520);
    ...
    enterStr.setCharacterSize(30);
    enterStr.setFillColor(Color::White);
    enterStr.setPosition(350, 330);

    Text Contactstxt;
    Contactstxt.setFont(font);
    Contactstxt.setString("Contacts:");
    Contactstxt.setCharacterSize(35);
    Contactstxt.setPosition(383, 300);
    Contactstxt.setFillColor(Color::Blue);

    Text addUsertxt;
    addUsertxt.setFont(font);
    addUsertxt.setCharacterSize(35);
    addUsertxt.setPosition(383, 500);
    addUsertxt.setFillColor(Color::Blue);
    ...
    for (int i = 0; i < 50; i++) {
        contactsText[i].setFillColor(Color(61, 144, 227));
        contactsText2[i].setFillColor(Color(61, 144, 227));
    }
    ...
    while (true) {
        ...
        if (Contacts) {
            ...
            while (contactsWindow.isOpen())
            {
                ...
                while (contactsWindow.pollEvent(event))
                {
                    ...
                    case Event::KeyPressed:
                        ...
                        {
                            contactsText[p].setFillColor(Color(61, 144, 227));
                            p++;
                            if (p > usernames.size())
                                p = 0;
                        ...
                        if (Keyboard::isKeyPressed(Keyboard::Enter)) {
                            if (isSearch) {
                                if (!searchVector.empty()) {
                                    contactUsername = searchVector[p];
                                }
                            }
                            else {
                                if (!usernames.empty()) {
                                    contactUsername = usernames[p];
                                }
                            }
                ...
                enterStr.setPosition(350, 330);
                enterStr.setFillColor(Color::Blue);
                ...
                if (isSearch && !str.empty()) {
                    ...
                    viewMyContacts(contactsText, contactsWindow, usernames, true, p);
            ...
            while (addContactWindow.isOpen())
            {
                    ...
                    case Event::MouseButtonPressed:
                        ...
                        if (Mx > 375 && Mx < 640 && My>625 && My < 720) {
                            ...
                            for (int i = 0; i < usernames.size(); i++) {
                                if (usernames[i] == str) {
                                    cout << "\n you already have this contact \ n";
                                    flag = false;
                                }
                            }
                            if (findUsername(users, str, currentUsername) && flag)
                            {
                                ifstream file;
                                file.open("files\\text.txt");
                                ...
                                if (newfile.is_open())
                                {
                                    ...
                                    newfile << str << '\n';
                                    addUsertxt.setString("new user is added\n");
                                    cout << "new user is added\n";
                                }
                                newfile.close();
                            }
                            else {
                                addUsertxt.setString("enter correct username");
                ...
                addContactWindow.draw(addUsertxt);
            ...
            while (Sendmessege.isOpen())
            {
                ...
                while (Sendmessege.pollEvent(event))
                {
                    ...
                    if (sf::Mouse::getPosition(Sendmessege).x >= 30
                        && sf::Mouse::getPosition(Sendmessege).y >= 520
                        && sf::Mouse::getPosition(Sendmessege).x <= 280
                        && sf::Mouse::getPosition(Sendmessege).y <= 595
                        ) {
                        contact_button.setScale(1.08, 1.08);
                        if (event.type == Event::MouseButtonPressed) {
                            viewContact = true;
                            sendmessage = false;
                            Sendmessege.close();
                ...
                Sendmessege.draw(contact_button);
            ...
            userContacts.clear();
            double id = getContactId(users, contactUsername);
            string ch;
            fstream my_file;
            my_file.open("files\\text.txt", ios::in);
            if (!my_file) {
            ...
            while (ViewContact.isOpen())
            {
                ...
                while (ViewContact.pollEvent(event))
                {
                    ...
                    if (sf::Mouse::getPosition(ViewContact).x >= 162 && sf::Mouse::getPosition(ViewContact).y >= 167
                        && sf::Mouse::getPosition(ViewContact).x <=
                        238 &&
